<!-- <template>
  <div class="p-6">
    <h1 class="text-2xl mb-4">Salesforce Accounts</h1>
    <div class="mb-4">
      <button @click="prevPage" :disabled="page === 1" class="border px-2 py-1">
        Prev
      </button>
      <span class="mx-2">Page {{ page }}</span>
      <button @click="nextPage" class="border px-2 py-1">Next</button>
    </div>
...
export default {
  ...
  async created() {
    ...
    if (newAccessToken && newInstanceUrl) {
      ...
      // Remove them from the URL so we don’t re‐process them
    ...
    async fetchAccounts() {
      try {
        const res = await axios.get(
          `https://useful-osprey-hideously.ngrok-free.app/api/accounts?page=${this.page}&limit=${this.limit}`,
          {
            headers: {
...
<!-- <template>
  <div
    class="min-h-screen flex flex-col bg-gradient-to-br from-purple-50 to-purple-100"
  >
    <header class="bg-purple-600 text-white p-4 flex justify-end">
    ...
    <div class="flex-grow p-6">
      ...
      <div v-if="isLoading" class="mb-4 text-gray-600">
        <span
          class="inline-block animate-spin rounded-full h-6 w-6 border-4 border-purple-400 border-t-transparent mr-2"
        ></span>
      ...
      <div class="mt-4 flex items-center">
        <button
          @click="prevPage"
          :disabled="page === 1"
          class="bg-purple-600 text-white px-3 py-1 rounded hover:bg-purple-700 transition mr-2"
        >
          Prev
        </button>
